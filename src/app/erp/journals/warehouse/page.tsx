...
export default function WarehouseJournal() {
  const [documents, setDocuments] = useState<ErpDocument[]>([]);
  const [search, setSearch] = useState('');

  useEffect(() => {
    const docs = getDocuments()
      .filter(d => d.type === 'WAREHOUSE')
  ...
  return (
    <div style={{ padding: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem' }}>
        <div>
          <h1 style={{ fontSize: '2rem', fontWeight: 800, color: 'var(--text-color)', marginBottom: '0.5rem' }}>Anbar əməliyyatları</h1>
          <p style={{ color: 'var(--text-secondary)' }}>Materialların alışı, silinməsi və yerdəyişməsi jurnalı.</p>
        </div>
      </div>
      ...
      <div style={{ backgroundColor: 'var(--surface-color)', border: '1px solid var(--border-color)', borderRadius: '20px', overflow: 'hidden' }}>
        <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--border-color)' }}>
          <div style={{ position: 'relative' }}>
            ...
            <input 
              type="text" 
              placeholder="Faktura nömrəsi və ya Anbar adı..." 
              value={search}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead>
              <tr style={{ backgroundColor: 'var(--bg-color)', borderBottom: '1px solid var(--border-color)' }}>
                ...
                <th style={{ padding: '1.25rem 1.5rem', color: 'var(--text-secondary)', fontWeight: 600, fontSize: '0.85rem' }}>Anbar / Şöbə</th>
            ...
            <tbody>
              {filtered.map((doc) => (
                <tr key={doc.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                  ...
                  <td style={{ padding: '1rem 1.5rem', fontWeight: 600, color: doc.name.includes('Alış') ? '#f59e0b' : 'var(--text-primary)' }}>{doc.name}</td>

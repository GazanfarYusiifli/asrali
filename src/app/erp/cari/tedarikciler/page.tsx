...
export default function TedarikcilerPage() {
  ...
  useEffect(() => {
    ...
    const suppliers = allCustomers.filter((c: any) => c.hesabNovu === 'Təchizatçı' || c.hesabNovu === 'Həm Müştəri, Həm Təchizatçı');
    suppliers.sort((a: any, b: any) => a.hesabAdi.localeCompare(b.hesabAdi));
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    const suppliers = updatedAll.filter((c: any) => c.hesabNovu === 'Təchizatçı' || c.hesabNovu === 'Həm Müştəri, Həm Təchizatçı');
    suppliers.sort((a: any, b: any) => a.hesabAdi.localeCompare(b.hesabAdi));
    setCustomers(suppliers);
    closeModal();
  };
                ...
                <tr key={c.id} style={{ borderBottom: '1px solid #e2e8f0', transition: 'background-color 0.2s' }} onMouseOver={e=>e.currentTarget.style.backgroundColor='#f8fafc'} onMouseOut={e=>e.currentTarget.style.backgroundColor='white'}>
                  ...
                  <td style={tdStyle}>
                    <Link href={`/erp/cari/musteriler/${c.id}`} style={{ textDecoration: 'none' }}>
                      <div style={{ fontWeight: 800, color: '#f59e0b', fontSize: '1rem', marginBottom: '0.2rem', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                  ...
                  <td style={tdStyle}>
                    <span style={{ padding: '4px 10px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 700, backgroundColor: c.hesabNovu === 'Təchizatçı' ? '#fef3c7' : '#e0e7ff', color: c.hesabNovu === 'Təchizatçı' ? '#d97706' : '#4f46e5' }}>
                      {c.hesabNovu}
                    </span>
                  </td>
          ...
          <div style={{ backgroundColor: 'white', borderRadius: '20px', width: '100%', maxWidth: '900px', maxHeight: '90vh', display: 'flex', flexDirection: 'column', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '1.5rem 2rem', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#f8fafc' }}>
              <h2 style={{ margin: 0, color: '#1e293b', fontSize: '1.4rem', fontWeight: 800 }}>
                {editingId ? 'Təchizatçını Redaktə Et' : 'Yeni Təchizatçı Əlavə Et'}
              </h2>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                      <div style={formGroup}>
                        ...
                        <input name="hesabAdi" value={formData.hesabAdi} onChange={handleInputChange} style={inputStyle} required placeholder="Tədarükçü şirkətin adı" />
                      </div>
                      <div style={formGroup}>
                        <label style={labelStyle}>Hesab Kodu</label>
                        <input name="hesabKodu" value={formData.hesabKodu} onChange={handleInputChange} style={inputStyle} placeholder="Məs: T-1001" />
                      </div>
                      <div style={formGroup}>
                    ...
                    <div style={formGroup}>
                      ...
                      <textarea name="ibanNotlar" value={formData.ibanNotlar} onChange={handleInputChange} style={{...inputStyle, minHeight: '120px'}} placeholder="Təchizatçının bank məlumatları, müqavilə şərtləri və s." />
                    </div>
                  </div>
                )}
                ...
                {activeTab === 'maliyye' && (
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                    <div style={{...formGroup, backgroundColor: 'white', padding: '1rem', borderRadius: '12px', border: '1px solid #e2e8f0'}}>
                      <label style={{...labelStyle, color: '#f59e0b'}}>Hesab Növü</label>
                    ...
                    <div style={formGroup}>
                      ...
                      <input name="isciTeyin" value={formData.isciTeyin} onChange={handleInputChange} style={inputStyle} placeholder="Təmsilçinin adı" />
                    </div>
                  </div>
                )}
                ...
                {activeTab === 'yaxinlari' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    <div style={{ backgroundColor: '#e0e7ff', color: '#4338ca', padding: '1rem', borderRadius: '12px', fontSize: '0.9rem', fontWeight: 600 }}>
                      Təchizatçının fövqəladə hal və ya alternativ əlaqə üçün yaxınlarının məlumatları (Ad, Soyad, Qohumluq dərəcəsi, Nömrə).
                    </div>
                    <div style={formGroup}>
                      <label style={labelStyle}>Yaxınları (Sərbəst Mətn)</label>
                      <textarea name="yaxinlari" value={formData.yaxinlari} onChange={handleInputChange} style={{...inputStyle, minHeight: '200px'}} placeholder="1. Əli Həsənov - 050 123 45 67&#10;2. ..." />
                    </div>
                  </div>
                )}
                ...
                {activeTab === 'zamin' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    <div style={{ backgroundColor: '#fef3c7', color: '#b45309', padding: '1rem', borderRadius: '12px', fontSize: '0.9rem', fontWeight: 600 }}>
                      Tədarüklərdə zamin (kefil) duran şəxsin detalları (VÖEN, Ad, Ünvan, İş yeri).
                    </div>
                    <div style={formGroup}>
            ...
            <div style={{ padding: '1.5rem 2rem', borderTop: '1px solid #e2e8f0', backgroundColor: 'white', display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
              ...
              <button type="submit" form="customerForm" style={{ padding: '0.8rem 2.5rem', backgroundColor: '#f59e0b', color: 'white', border: 'none', borderRadius: '12px', fontWeight: 700, fontSize: '1rem', cursor: 'pointer', boxShadow: '0 4px 14px 0 rgba(245, 158, 11, 0.39)', transition: 'background-color 0.2s' }} onMouseOver={e=>e.currentTarget.style.backgroundColor='#d97706'} onMouseOut={e=>e.currentTarget.style.backgroundColor='#f59e0b'}>{editingId ? 'Yadda Saxla' : 'Təchizatçını Yarat'}</button>
            </div>
          </div>
        </div>
      )}

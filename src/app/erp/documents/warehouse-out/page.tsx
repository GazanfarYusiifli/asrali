...
function WarehouseOutCreate({ onClose }: { onClose: () => void }) {
  const [activeRowId, setActiveRowId] = useState<string | null>(null);

  const handlePost = (totalAmount: number, docName: string, rows: DocumentRow[], counterparty?: CatalogItem) => {
    const cpName = counterparty ? counterparty.name : 'Təyinat';
    rows.forEach(r => {
      if (r.amount > 0) {
        addPosting({ date: new Date().toISOString(), dt: '701', ct: '201', amount: r.amount, document: docName, desc: `${cpName} - Çıxış: ${r.catalogItem?.name || r.desc}` });
      }
    });
  };

  return (
    <DocumentLayout title="Anbardan Çıxış Aktı" type="WAREHOUSE" prefix="ANB-C" counterpartyCatalog="warehouses" onPost={handlePost} onClose={onClose}>
      {({ rows, setRows }) => {
...
export default function WarehouseOutPage() {
  ...
  if (view === 'create') return <WarehouseOutCreate onClose={() => setView('list')} />;
  return <DocumentList title="Anbardan Çıxış Aktı" onCreate={() => setView('create')} />;
}

...
function PayrollCreate({ onClose }: { onClose: () => void }) {
  const [activeRowId, setActiveRowId] = useState<string | null>(null);

  const handlePost = (totalAmount: number, docName: string, rows: DocumentRow[], counterparty?: CatalogItem) => {
    rows.forEach(r => {
      if (r.amount > 0) {
        addPosting({ date: new Date().toISOString(), dt: '711', ct: '545', amount: r.amount, document: docName, desc: `Maaş: ${r.catalogItem?.name || r.desc}` });
      }
    });
  };

  return (
    <DocumentLayout title="Əməkhaqqı Hesablanması (Tabelyar)" type="SERVICE" prefix="EMH" onPost={handlePost} onClose={onClose}>
      {({ rows, setRows }) => {
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              <thead>
                <tr style={{ backgroundColor: 'var(--surface-color)', borderBottom: '1px solid var(--border-color)' }}>
                  ...
                  <th style={{ padding: '1rem', color: 'var(--text-secondary)' }}>İşçi (Kataloq)</th>
                  <th style={{ padding: '1rem', color: 'var(--text-secondary)', width: '200px' }}>Hesablanmış Maaş (₼)</th>
              ...
              <tbody>
                {rows.map((row, index) => (
                  <tr key={row.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                    ...
                    <td style={{ padding: '0.5rem 1rem' }}>
                      <input type="number" min="0" value={row.amount} onChange={e => updateRow(row.id, 'amount', Number(e.target.value))} style={{ width: '100%', padding: '0.6rem', backgroundColor: 'var(--bg-color)', border: '1px solid var(--border-color)', borderRadius: '6px', fontWeight: 800 }} />
                    </td>
              ...
              <LookupModal 
                ...
                onSelect={(item) => {
                  setRows(rows.map(r => {
                    ...
                    return { ...r, catalogItem: item, desc: item.name };
                  }));
                  setActiveRowId(null);
                }}
              />
...
export default function PayrollPage() {
  ...
  if (view === 'create') return <PayrollCreate onClose={() => setView('list')} />;
  return <DocumentList title="Əməkhaqqı Hesablanması (Tabelyar)" onCreate={() => setView('create')} />;
}

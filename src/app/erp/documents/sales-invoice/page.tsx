...
function SalesInvoiceCreate({ onClose }: { onClose: () => void }) {
  ...
  const handlePost = (totalAmount: number, docName: string, rows: DocumentRow[], counterparty?: CatalogItem) => {
    ...
    rows.forEach(r => {
      if (r.amount > 0) {
        addPosting({ date: new Date().toISOString(), dt: '211', ct: '601', amount: r.amount, document: docName, desc: `${cpName} - Satış: ${r.catalogItem?.name || r.desc}` });
        const cost = (r.catalogItem?.properties?.cost || 0) * r.qty;
        addPosting({ date: new Date().toISOString(), dt: '701', ct: '201', amount: cost, document: docName, desc: `Maya dəyəri: ${r.catalogItem?.name || r.desc}` });
      }
    });
  };

  return (
    <DocumentLayout title="Satış Fakturası" type="SERVICE" prefix="SF" counterpartyCatalog="patients" onPost={handlePost} onClose={onClose}>
      {({ rows, setRows }) => {
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              <thead>
                <tr style={{ backgroundColor: 'var(--surface-color)', borderBottom: '1px solid var(--border-color)' }}>
                  ...
                  <th style={{ padding: '1rem', color: 'var(--text-secondary)' }}>Mal / Nomenklatura</th>
              ...
              <LookupModal 
                ...
                onSelect={(item) => {
                  setRows(rows.map(r => {
                    ...
                    // For sales, we might not have a fixed sales price, just use cost + 20% mock
                    const cost = item.properties?.cost || 0;
...
export default function SalesInvoicePage() {
  ...
  if (view === 'create') return <SalesInvoiceCreate onClose={() => setView('list')} />;
  return <DocumentList title="Satış Fakturası" onCreate={() => setView('create')} />;
}

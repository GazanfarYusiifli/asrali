...
function PurchaseInvoiceCreate({ onClose }: { onClose: () => void }) {
  const [activeRowId, setActiveRowId] = useState<string | null>(null);

  const handlePost = (totalAmount: number, docName: string, rows: DocumentRow[], counterparty?: CatalogItem) => {
    const cpName = counterparty ? counterparty.name : 'Təchizatçı';
    rows.forEach(r => {
      if (r.amount > 0) {
        addPosting({ date: new Date().toISOString(), dt: '201', ct: '531', amount: r.amount, document: docName, desc: `${cpName} - Alış: ${r.catalogItem?.name || r.desc}` });
      }
    });
  };

  return (
    <DocumentLayout title="Alış Fakturası" type="WAREHOUSE" prefix="AF" counterpartyCatalog="counterparties" onPost={handlePost} onClose={onClose}>
      {({ rows, setRows }) => {
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              <thead>
                <tr style={{ backgroundColor: 'var(--surface-color)', borderBottom: '1px solid var(--border-color)' }}>
                  ...
                  <th style={{ padding: '1rem', color: 'var(--text-secondary)', width: '150px' }}>Alış Qiyməti (₼)</th>
...
export default function PurchaseInvoicePage() {
  ...
  if (view === 'create') return <PurchaseInvoiceCreate onClose={() => setView('list')} />;
  return <DocumentList title="Alış Fakturası" onCreate={() => setView('create')} />;
}

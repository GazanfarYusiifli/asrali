...
function CashOutCreate({ onClose }: { onClose: () => void }) {
  const handlePost = (totalAmount: number, docName: string, rows: DocumentRow[], counterparty?: CatalogItem) => {
    ...
    rows.forEach(r => {
      if (r.amount > 0) {
        addPosting({ date: new Date().toISOString(), dt: '531', ct: '221', amount: r.amount, document: docName, desc: `${cpName} - ${r.desc}` });
      }
    });
  };

  return (
    <DocumentLayout title="Kassa Məxaric Orderi" type="CASH" prefix="KMX" counterpartyCatalog="counterparties" onPost={handlePost} onClose={onClose}>
      {({ rows, setRows }) => {
                ...
                {rows.map((row, index) => (
                  <tr key={row.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                    ...
                    <td style={{ padding: '0.5rem 1rem' }}>
                      <input type="text" value={row.desc} onChange={e => updateRow(row.id, 'desc', e.target.value)} placeholder="Məs: Təchizatçıya ödəniş" style={{ width: '100%', padding: '0.6rem', backgroundColor: 'var(--bg-color)', border: '1px solid var(--border-color)', borderRadius: '6px' }} />
                    </td>
                    <td style={{ padding: '0.5rem 1rem' }}>
...
export default function CashOutPage() {
  ...
  if (view === 'create') return <CashOutCreate onClose={() => setView('list')} />;
  return <DocumentList title="Kassa Məxaric Orderi" onCreate={() => setView('create')} />;
}

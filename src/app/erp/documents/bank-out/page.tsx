...
function BankOutCreate({ onClose }: { onClose: () => void }) {
  const handlePost = (totalAmount: number, docName: string, rows: DocumentRow[], counterparty?: CatalogItem) => {
    ...
    rows.forEach(r => {
      if (r.amount > 0) {
        addPosting({ date: new Date().toISOString(), dt: '531', ct: '223', amount: r.amount, document: docName, desc: `${cpName} - ${r.desc}` });
      }
    });
  };

  return (
    <DocumentLayout title="Bank Məxaric (Köçürmə)" type="BANK" prefix="BMX" counterpartyCatalog="counterparties" onPost={handlePost} onClose={onClose}>
      {({ rows, setRows }) => {
                ...
                {rows.map((row, index) => (
                  <tr key={row.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                    ...
                    <td style={{ padding: '0.5rem 1rem' }}>
                      <input type="text" value={row.desc} onChange={e => updateRow(row.id, 'desc', e.target.value)} placeholder="Məs: Vergi ödənişi" style={{ width: '100%', padding: '0.6rem', backgroundColor: 'var(--bg-color)', border: '1px solid var(--border-color)', borderRadius: '6px' }} />
                    </td>
                    <td style={{ padding: '0.5rem 1rem' }}>
                      <input type="number" min="0" value={row.amount} onChange={e => updateRow(row.id, 'amount', Number(e.target.value))} style={{ width: '100%', padding: '0.6rem', backgroundColor: 'var(--bg-color)', border: '1px solid var(--border-color)', borderRadius: '6px', color: '#ef4444', fontWeight: 800 }} />
                    </td>
...
export default function BankOutPage() {
  const [view, setView] = React.useState<'list' | 'create'>('list');
  if (view === 'create') return <BankOutCreate onClose={() => setView('list')} />;
  return <DocumentList title="Bank Məxaric (Köçürmə)" onCreate={() => setView('create')} />;
}

...
function BankInCreate({ onClose }: { onClose: () => void }) {
  const handlePost = (totalAmount: number, docName: string, rows: DocumentRow[], counterparty?: CatalogItem) => {
    ...
    rows.forEach(r => {
      if (r.amount > 0) {
        addPosting({ date: new Date().toISOString(), dt: '223', ct: '211', amount: r.amount, document: docName, desc: `${cpName} - ${r.desc}` });
      }
    });
  };

  return (
    <DocumentLayout title="Bank Mədaxil (Çıxarış)" type="BANK" prefix="BMD" counterpartyCatalog="patients" onPost={handlePost} onClose={onClose}>
      {({ rows, setRows }) => {
                ...
                {rows.map((row, index) => (
                  <tr key={row.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                    ...
                    <td style={{ padding: '0.5rem 1rem' }}>
                      <input type="text" value={row.desc} onChange={e => updateRow(row.id, 'desc', e.target.value)} placeholder="Məs: Sığorta ödənişi" style={{ width: '100%', padding: '0.6rem', backgroundColor: 'var(--bg-color)', border: '1px solid var(--border-color)', borderRadius: '6px' }} />
                    </td>
                    <td style={{ padding: '0.5rem 1rem' }}>
                      <input type="number" min="0" value={row.amount} onChange={e => updateRow(row.id, 'amount', Number(e.target.value))} style={{ width: '100%', padding: '0.6rem', backgroundColor: 'var(--bg-color)', border: '1px solid var(--border-color)', borderRadius: '6px', color: '#3b82f6', fontWeight: 800 }} />
                    </td>
...
export default function BankInPage() {
  const [view, setView] = React.useState<'list' | 'create'>('list');
  if (view === 'create') return <BankInCreate onClose={() => setView('list')} />;
  return <DocumentList title="Bank Mədaxil (Çıxarış)" onCreate={() => setView('create')} />;
}

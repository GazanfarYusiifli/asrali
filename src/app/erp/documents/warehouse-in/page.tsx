...
function WarehouseInCreate({ onClose }: { onClose: () => void }) {
  const [activeRowId, setActiveRowId] = useState<string | null>(null);

  const handlePost = (totalAmount: number, docName: string, rows: DocumentRow[], counterparty?: CatalogItem) => {
    const cpName = counterparty ? counterparty.name : 'Mənbə';
    rows.forEach(r => {
      if (r.amount > 0) {
        addPosting({ date: new Date().toISOString(), dt: '201', ct: '531', amount: r.amount, document: docName, desc: `${cpName} - Qəbul: ${r.catalogItem?.name || r.desc}` });
      }
    });
  };

  return (
    <DocumentLayout title="Anbara Mədaxil Aktı" type="WAREHOUSE" prefix="ANB-G" counterpartyCatalog="counterparties" onPost={handlePost} onClose={onClose}>
      {({ rows, setRows }) => {
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
              <thead>
                <tr style={{ backgroundColor: 'var(--surface-color)', borderBottom: '1px solid var(--border-color)' }}>
                  ...
                  <th style={{ padding: '1rem', color: 'var(--text-secondary)' }}>Material (Nomenklatura)</th>
                  <th style={{ padding: '1rem', color: 'var(--text-secondary)', width: '100px' }}>Sayı</th>
                  <th style={{ padding: '1rem', color: 'var(--text-secondary)', width: '150px' }}>Dəyəri (₼)</th>
...
export default function WarehouseInPage() {
  ...
  if (view === 'create') return <WarehouseInCreate onClose={() => setView('list')} />;
  return <DocumentList title="Anbara Mədaxil Aktı" onCreate={() => setView('create')} />;
}

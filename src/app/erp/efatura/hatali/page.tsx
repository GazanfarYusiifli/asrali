...
export default function XetaliFakturalar() {
  ...
  useEffect(() => {
    ...
    if (data) {
      ...
      setInvoices(parsed.filter((inv: any) => inv.status === 'ERROR'));
    }
  }, []);
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead style={{ position: 'sticky', top: 0, backgroundColor: '#f8fafc', zIndex: 10 }}>
              <tr style={{ borderBottom: '1px solid #e2e8f0' }}>
                ...
                <th style={{ padding: '1rem 1.5rem', color: '#475569', fontWeight: 700, fontSize: '0.85rem' }}>Xəta Tipi</th>
              ...
              {filteredInvoices.length === 0 ? (
                ...
                filteredInvoices.map((inv) => (
                  <tr key={inv.id} style={{ borderBottom: '1px solid #e2e8f0', transition: 'background-color 0.2s' }} onMouseOver={e => e.currentTarget.style.backgroundColor = '#f1f5f9'} onMouseOut={e => e.currentTarget.style.backgroundColor = 'transparent'}>
                    ...
                    <td style={{ padding: '1rem 1.5rem', fontWeight: 800, color: '#d97706' }}>VÖEN Xətası</td>

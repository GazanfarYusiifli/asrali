...
export default function AlisRaporuPage() {
  ...
  useEffect(() => {
    ...
    // Fetch Alış Siyahısı records
    const purchaseData = JSON.parse(getAppStorage('erp_purchases') || '[]');
    const formattedPurchases = purchaseData.map((item: any) => {
      ...
      return {
        ...
        hesabKodu: '-',
        techizatciAdi: item.hesapAdi || 'Adsız',
    ...
    // Sort (Newest first)
    const sorted = formattedPurchases.sort((a: any, b: any) => b.timestamp - a.timestamp);
    setPurchaseRecords(sorted);
  }, []);

  if (!isMounted) return null;

  // Derived Filtered Data
  const filteredRecords = purchaseRecords.filter(item => {
    // 1. Search Term
    if (searchTerm) {
      const lowerSearch = searchTerm.toLowerCase();
      const matchesSearch = 
        item.techizatciAdi?.toLowerCase().includes(lowerSearch) ||
    ...
    // 3. Supplier Filter
    if (filters.supplier && !item.techizatciAdi?.toLowerCase().includes(filters.supplier.toLowerCase())) {
      return false;
    }
  ...
  // Date formatting for the current period
  ...
  return (
    <div style={{ padding: '2rem', height: '100%', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
        ...
        <div style={cardStyle}>
          <div style={cardIconWrapperStyle('#fdf4ff', '#d946ef')}><Calendar size={20} /></div>
          <div>
        ...
        <div style={cardStyle}>
          ...
          <div>
            <div style={cardLabelStyle}>ÜMUMİ XƏRC</div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '1200px' }}>
            <thead>
              <tr style={{ backgroundColor: '#ffffff', borderBottom: '2px solid #e2e8f0' }}>
                ...
                <th style={thStyle}>TƏCHİZATÇI ADI</th>
                ...
                <tr>
                  <td colSpan={11} style={{ padding: '5rem 2rem', textAlign: 'center', backgroundColor: '#fafafa' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1rem' }}>
                      <div style={{ padding: '1.5rem', backgroundColor: '#fef2f2', borderRadius: '50%', color: '#ef4444' }}>
                        <FileText size={48} strokeWidth={1.5} />
                      </div>
                      <h3 style={{ fontSize: '1.2rem', fontWeight: 700, color: '#334155', margin: 0 }}>Alış tapılmadı</h3>
                      <p style={{ color: '#64748b', margin: 0, maxWidth: '400px', lineHeight: '1.5' }}>
                        Seçdiyiniz filtrlərə uyğun alış qeydi yoxdur. Zəhmət olmasa filtrləri təmizləyin və ya yeni alış yaradın.
                      </p>
                    </div>
                  </td>
                </tr>
          ...
          <div style={{ width: '500px', backgroundColor: 'white', borderRadius: '20px', padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1), 0 10px 10px -5px rgba(0,0,0,0.04)', maxHeight: '90vh' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #f1f5f9', paddingBottom: '1rem' }}>
              <h2 style={{ fontSize: '1.3rem', fontWeight: 800, color: '#0f172a', display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                <div style={{ backgroundColor: '#fee2e2', padding: '0.5rem', borderRadius: '8px', color: '#ef4444', display: 'flex' }}><Filter size={18} /></div> 
                Geniş Filtrlər (Alışlar)
              </h2>
            ...
            <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '1.2rem', paddingRight: '0.5rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 700, color: '#475569' }}>Təchizatçı Filtri</label>
                <input type="text" value={filters.supplier} onChange={e => setFilters({...filters, supplier: e.target.value})} placeholder="Təchizatçı adı və ya kodu ilə axtar..." style={inputStyle} />
              </div>
            ...
            <div style={{ display: 'flex', gap: '1rem', marginTop: '0.5rem', paddingTop: '1.5rem', borderTop: '1px solid #f1f5f9' }}>
              <button 
                onClick={() => setFilters({ startDate: '', endDate: '', supplier: '', branch: 'Bütün Filiallar', currency: 'Bütün Valyutalar', status: 'Hamısı', ecommerce: 'Hamısını Göstər', tag: 'Etiket seçin...' })} 
              ...
              <button 
                ...
                style={{ flex: 1, padding: '0.8rem', backgroundColor: '#ef4444', color: 'white', border: 'none', borderRadius: '10px', fontWeight: 700, cursor: 'pointer', transition: 'all 0.2s', boxShadow: '0 4px 6px -1px rgba(239, 68, 68, 0.2)' }} 
